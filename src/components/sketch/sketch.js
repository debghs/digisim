  ...
  const Sketch = (p) => {
    ...
    class Chip {
      constructor(x, y, concept) {
        ...
        this.exec = !this.definedByCircuit ? concept.exec : null;
        this.circuit = this.definedByCircuit ? concept : null;
      ...
      contains(x, y) {
        return p.dist(this.x, this.y, x, y) < p.max(this.w, this.h) / 2;
      }
      ...
      render() {
        ...
        p.rectMode(p.CENTER);
        p.rect(this.x, this.y, this.w, this.h);
        if (selectedChip === this) {
          p.stroke(255, 0, 0); // Red border for selected chip
          p.strokeWeight(3);
          p.noFill();
          p.rect(this.x, this.y, this.w, this.h);
        }
    ...
    class Wire {
      ...
      contains(x, y) {
        const threshold = 10; // Threshold distance to consider a click on the wire
        const d1 = p.dist(x, y, this.a.x, this.a.y);
        const d2 = p.dist(x, y, this.b.x, this.b.y);
        const distance = p.dist(this.a.x, this.a.y, this.b.x, this.b.y);
        const buffer = 10; // Additional distance to allow for thicker wires
    
        if (d1 < threshold || d2 < threshold) return true;
        if (distance - d1 - d2 < buffer) return true;
    ...
    p.mousePressed = (e) => {
      ...
      if (e.ctrlKey && e.shiftKey) {
        if (selectedChip) {
          const index = chips.indexOf(selectedChip);
      ...
      for (const chip of chips) {
        if (chip.contains(p.mouseX, p.mouseY)) {
          if (e.ctrlKey) {
            selectedChip = chip;
            return;
          }
      ...
      for (const wire of wires) {
        if (wire.contains(p.mouseX, p.mouseY)) {
          if (e.ctrlKey) {
            selectedWire = wire;
            return;
          }
